...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define	INRUSH_RELAY_DELAY_MS				1000	// length of inrush delay in milliseconds
#define	INRUSH_BUS_VOLTAGE_ON_THRESHOLD		125		// 250 Vdc
#define	INRUSH_BUS_VOLTAGE_OFF_THRESHOLD	100		// 200 Vdc

#define	FALSE								0
#define TRUE								1

#define	INRUSH_RELAY_INACTIVE	0
#define	INRUSH_RELAY_ACTIVE		1
#define	RELAY_PIN_RESET			0
#define	RELAY_PIN_SET			1
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
unsigned short	inrush_relay_state = 0;
unsigned short	relay_pin_state = 0;
unsigned short	inrush_relay_activate_time = 0;

extern	RDivider_Handle_t * pBusSensorM1;
extern	RDivider_Handle_t RealBusVoltageSensorParamsM1;
extern	MCT_Handle_t MCT[];
/* USER CODE END PV */
...
//	Bus_Voltage_D = VBS_GetAvBusVoltage_d(MCT[0].pBusVoltageSensor);
	...
	//bus_voltage_d = Bus_Voltage_D;
//  	Bus_Voltage_V = VBS_GetAvBusVoltage_V(MCT[0].pBusVoltageSensor);
//	Bus_Voltage_V = VBS_GetAvBusVoltage_V(UI_Params.pMCT[0].pBusVoltageSensor);
    ...
    if((inrush_relay_state == INRUSH_RELAY_ACTIVE) && (bus_voltage <= (INRUSH_BUS_VOLTAGE_OFF_THRESHOLD))) {
    	inrush_relay_state = INRUSH_RELAY_INACTIVE;
		isTicking = FALSE;
		// deactivate relay
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  LL_ADC_SetChannelSamplingTime(ADC1, LL_ADC_CHANNEL_11, LL_ADC_SAMPLINGTIME_4CYCLES_5);
...
static void MX_TIM1_Init(void)
{
  ...
  TIM_BDTRInitStruct.Break2Filter = LL_TIM_BREAK2_FILTER_FDIV1_N4;
